...
- **Auth circuit breaker**: Worker uses exact error signatures (`AUTH_ERROR_SIGNATURES` in `item-detail-worker.ts`) to detect auth failures. On match, marks all remaining DISCOVERED items as ERROR to prevent cascading failures.
- **Session resume auto-refresh**: Reprocess API sets `completedAt: null` on the session. Session page `isActive` checks for `(status === "COMPLETED" && !completedAt)` to enable auto-refresh during reprocessing. When the detail worker finishes the last item (`itemsProcessed === itemsFound`), it restores `completedAt: new Date()` so auto-refresh stops.
