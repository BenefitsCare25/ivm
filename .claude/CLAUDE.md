...
- **Auth**: Cookie injection (Chrome Extension capture) or credential login. `resolveAuth()` tries cookies first, detects login page after navigation, falls back to credentials if login detected. `isLoginPage()` exported for per-item auth checks. **`resolveAuth` ignores `authMethod`** — it uses whatever the credential row holds (cookies first, then username/password), so a portal can store BOTH and the credential login acts as an automatic fallback. **Called per-item** in the detail worker, so this fallback IS the mid-scrape self-heal: if the cookie session dies mid-run, the next item re-logs-in and the run continues (no circuit-break) — provided credentials are stored. On a credential re-login, `persistFreshCookies(portalId, …)` writes the fresh cookies + a 24h expiry back to the portal so only the first post-expiry item pays the full-login cost; pass `portalId` to `resolveAuth` to enable this (wired in `item-detail-worker`/`portal-worker`; omitted by one-off analyze/discovery callers). **UI**: `PortalAuthPanel` lets a cookie-auth portal ALSO store login credentials (collapsible "Add login credentials" section) — the `/api/portals/[id]/credentials` upsert never touches `authMethod`. **Cookie-only = cannot self-heal** (cookies die and can't be renewed without a login); storing the IVM account's credentials is what makes a scrape robust against mid-run logout. Caveat: whether a given portal's login form automates cleanly (vs SSO/captcha) must be verified with a real login. **`isLoginPage()` detects the unauthenticated state three ways**: (1) login URL patterns, (2) **logout/session-ended URL patterns** (`/logout`, `/signout`, `/logged-out`, `session-expired` — a dead session often redirects to a logout interstitial like Inspro's `/admin/logout`, which has NO password field and no "login" in the URL), (3) password input on an off-path page, and (4) a short-page body-text scan for "logged out / session expired / back to login" markers (bounded to ≤2000 chars so a data-heavy authenticated page carrying a "Logout" nav link is never misread). Before this, a logout redirect was read as "still authenticated", so keep-alive kept refreshing dead cookies.
...
- **Runtime auth expiration detection**: `ScrapeSession.authExpiredAt` (nullable DateTime) is set by the circuit breaker when the portal invalidates the session server-side before cookies technically expire. Session page computes `effectiveAuthStatus`: if `authExpiredAt` is set and static `authStatus` is `"ok"`, overrides to `"session_expired"`. `AuthStatus` type includes `"session_expired"`. `SessionActions` shows a red banner and blocks auto-retry while `authBad` (`expired|session_expired|missing`). Reprocess API clears `authExpiredAt: null` when user retries after fixing cookies.
- **Resume-deadlock fix (graceful pause/resume)**: the Retry/Continue buttons gate on `credentialBad` (`expired|missing`), NOT `authBad`. A `session_expired` session means the credential is valid again (cookies re-captured / present) but the session still carries a stale `authExpiredAt`; the ONLY thing that clears that flag is the reprocess API, reachable only via those buttons — so gating them on `authBad` deadlocked the session (re-capture → still can't resume). Now: mid-run auth loss pauses (banner "session expired mid-scrape — re-capture then Continue"), user re-captures cookies, clicks Continue/Retry, reprocess clears `authExpiredAt` + resumes with fresh cookies. Auto-retry still skips `authBad` (no auto-hammer into possibly-dead cookies). `assertAuthValid` already permitted this server-side (checks live credential only). This is the graceful pause/resume for cookie-only portals with no credential self-heal.
- **Captured-cookie lifetime (`src/lib/cookie-expiry.ts`)**: `deriveCookieExpiresAt(cookies)` sets `cookieExpiresAt` from the captured cookies' real `expires` (unix seconds) — the furthest-out persistent expiry (a "remember me" token), capped 30d, falling back to 24h for session-only cookies. Wired into BOTH cookie-save routes (`api/portals/[id]/cookies`, `api/extension/cookies`). Fixes `resolveAuth` needlessly dropping a still-valid long-lived session at the old hardcoded 24h. Over-long values degrade gracefully — `isLoginPage` governs real liveness at scrape/keep-alive time.
...
- **Rule 1 — Government subsidy/deduction → flag**: `detectSubsidyDeduction()` scans document signal text for **MediSave / CPF, CHAS, or CDC (Community Development Council) voucher**. **Segment-scoped** (splits text into per-line/sentence segments): a keyword only counts when its segment carries a genuine deduction/payment context (`deduct|less|subsid|paid by/from/via|payment by/mode|utilis|claimed|contribution`) **or a dollar amount**, AND is **not** CPF instructional boilerplate (`CPF_BOILERPLATE_RE` — "VIEW YOUR MEDISAVE… CLAIM DETAILS ONLINE", "REIMBURSEMENT INFORMATION…", "cpf.gov.sg", "Integrated Shield Plan", etc.). This kills the false positive where the standard SG-invoice CPF footer (present on nearly every medical/dental bill) tripped a bare `\bmedisave\b` match. Emits `SUBSIDY_DEDUCTION` FAIL + FLAGs (subsidised portion not claimable).
...
- **Caveat (documented behavior, not a bug)**: a ping resets an *idle* timeout and re-syncs IVM's clock, but cannot defeat a portal's *absolute* session lifetime — in that case the sweep detects the death (login/logout page via `isLoginPage`) and marks expired; cookies must be re-captured. `authExpiredAt` is per-`ScrapeSession` (runtime circuit breaker), so keep-alive signals liveness via the credential's `cookieExpiresAt` instead.
- **Credential-based auto re-login (self-heal)**: when a sweep finds a dead session (login/logout page) AND the portal has stored credentials (`encryptedUsername`/`encryptedPassword`), `keepAliveOne` → `tryCredentialRelogin()` logs back in (`authenticateWithCredentials`, loginUrl = `baseUrl`, default form selectors — mirrors `resolveAuth`), confirms success by loading the list page through `isLoginPage`, captures fresh cookies, and persists them (`persistAlive`, +48h). Best-effort: never throws, returns `null` on bad/absent creds or a changed login form, in which case the item is marked expired. Sweep summary counts `reloggedIn`. **Cookie-only portals still can't self-heal** — all Inspro (`benefits.inspro.com.sg`) portals are currently cookie-only (Chrome-Extension capture, no stored creds), so for them a dead session still means re-capture; add credentials to a portal to enable self-heal. Caveat: repeated failed re-logins (wrong creds) retry every sweep — a lockout risk on strict portals; fix the creds or disable keep-alive. **Historical bug (fixed 2026-07-17)**: `isLoginPage` didn't recognise Inspro's `/admin/logout` interstitial, so the sweep read every dead session as "alive", re-saved dead cookies, and bumped `cookieExpiresAt` +48h — the UI showed "ok" for 2 days on an account that was actually logged out. Fixed by logout-page detection in `isLoginPage`.
- **⚠️ Single-session portals — keep-alive evicts the user (Inspro)**: `benefits.inspro.com.sg` allows only ONE active session per login. The server-side keep-alive ping (from the Azure VM) re-claims the session and **logs out the user's own browser**; IVM keeps re-saving its refreshed cookie so it always reports `expired=0` while the human is kicked. Pinging *more often* kicks the user *faster* — do NOT raise frequency to "fix" logouts on such portals. **Off switch**: `PORTAL_KEEPALIVE_MINUTES=off` (also `false`/`disabled`/`0`/negative) → `scheduleKeepAlive` removes the repeatable job and skips scheduling (`isKeepAliveDisabled()` in `keepalive-queue.ts`). Currently **disabled in prod** for the Inspro fleet. **Correct fix = a dedicated IVM login** (a separate Inspro account used only by IVM): capture that account's cookies, then keep-alive can run without contending with the human. Re-enable by setting `PORTAL_KEEPALIVE_MINUTES` back to a minutes value once IVM has its own account.
