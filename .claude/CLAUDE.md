...
- **No auto re-login for cookie-only portals**: keep-alive can only *detect + surface* expiry, not self-heal, unless the portal has stored credentials (`encryptedUsername`/`encryptedPassword`). All Inspro (`benefits.inspro.com.sg`) portals are cookie-only (Chrome-Extension capture, no stored creds), so when Inspro's server-side session ends the only remedy is re-capturing cookies. Credential-based auto re-login in the sweep is a future enhancement gated on a portal actually having credentials. **Historical bug (fixed 2026-07-17)**: `isLoginPage` didn't recognise Inspro's `/admin/logout` interstitial, so the sweep read every dead session as "alive", re-saved dead cookies, and bumped `cookieExpiresAt` +48h — the UI showed "ok" for 2 days on an account that was actually logged out. Fixed by logout-page detection in `isLoginPage`.
