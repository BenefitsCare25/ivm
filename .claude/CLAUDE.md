...
- **Rule 2 — Flex Polyclinic claim with a hospital bill → wrong claim type**: gated to **flex claims** (`isFlexClaim()` — portal name/base URL contains "flex"). Fires when ANY candidate claim-type value (all grouping fields via `resolveClaimTypeValues` + claim-type-labelled fields — NOT just the first grouping field) matches `/poly clinic|pcn/i` AND the **billing document's provider** is a known SG hospital. Hospital search text is built by `buildHospitalSearchText(fileExtractions, recognizedDocs)` — scoped to bill/receipt-family docs' provider fields (falls back to a bill's full text only when it has no provider field), so a referral letter or incidental address mention no longer false-positives. `detectHospital()` (`src/lib/reference/sg-hospitals.ts`) does a space-bounded, longest-name-first match over govt + private lists. Emits `WRONG_CLAIM_TYPE` FAIL + FLAGs. **Triggers on BOTH govt and private hospitals**; message names the hospital + kind.
- **Preserve-prior interaction**: both policy checks run ONLY inside the `!preservedPrior` branch of `runComparison` — a degraded re-run (a claimant's document transiently failed to extract) must not flip a previously-good item to "pending document" / "wrong claim type". Currency amounts appearing on BOTH the document and portal are deduped (portal pass skips keys already emitted by the document pass).
