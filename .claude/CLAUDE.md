...
- **Job timeout race fix**: `withTimeout()` accepts an `onTimeout` callback. On timeout it calls `handleFinalFailure` (sets DB to ERROR + increments `itemsProcessed`) before rejecting, so the BullMQ slot isn't freed until the DB is updated — preventing the brief "N+1 PROCESSING" display glitch.
...
- **Auth pre-flight validation**: `assertAuthValid()` from `src/lib/portal-auth.ts` — shared helper used by scrape start and reprocess APIs. Returns 400 ValidationError if cookies expired without credential fallback or no auth configured. `AuthStatus` type exported from `src/types/portal.ts`. Session page computes `authStatus` and passes to `SessionActions`. "Continue", "Retry", and "Scrape Now" buttons disabled with tooltip when auth is bad.
- **Auth circuit breaker**: Worker uses exact error signatures (`AUTH_ERROR_SIGNATURES` in `item-detail-worker.ts`) to detect auth failures. On match, marks all remaining DISCOVERED items as ERROR AND increments `itemsProcessed` by the cancelled count so `finalizeIfComplete` fires correctly.
...
- **Portal card item count**: Shows `lastSessionProcessed / lastSessionFound processed` (e.g. "26 / 26 processed") sourced from the most recent `ScrapeSession`. `PortalSummary` carries `lastSessionProcessed` and `lastSessionFound` (not a cumulative `totalItems`). Hidden when `lastSessionFound === 0` and no status badge.
