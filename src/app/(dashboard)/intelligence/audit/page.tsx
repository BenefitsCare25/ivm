...
export default async function IntelligenceAuditPage() {
  ...
  const validations = trackedItemIds.length > 0
    ? await db.validationResult.findMany({
        where: { trackedItemId: { in: trackedItemIds } },
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Results from document validation rules run during Portal Tracker processing.
        </p>
      </div>
      ...
      <div className="rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm text-muted-foreground">
        ...
        <div className="space-y-1.5">
          ...
          <ul className="list-disc pl-4 space-y-0.5 font-mono text-xs">
            ...
            <li><span className="text-foreground">MISSING_DOC</span> — required document absent from a document set</li>
          ...
          <p className="font-mono text-xs mt-1">
            Validations come from <span className="font-medium not-italic text-foreground">Portal Tracker</span> item
            processing. Use this log to debug rules that are not triggering as expected.
          </p>
        </div>
      </div>
      ...
      {validations.length === 0 ? (
        <EmptyState
          ...
          description="Process documents in Portal Tracker to see validation results here."
        />
      ) : (
