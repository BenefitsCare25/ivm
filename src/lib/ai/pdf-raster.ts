 ...
 * from node-canvas (already a dependency). The result is memoised as a PROMISE
 * so concurrent rasterizations all await the same load — a plain boolean flag
 * would let a second caller proceed before the async import finished polyfilling.
 */
