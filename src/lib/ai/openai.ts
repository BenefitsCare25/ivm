...
export async function extractWithOpenAI(request: AIExtractionRequest): Promise<AIExtractionResponse> {
  ...
  // (still under the detail worker's 10-min job budget). Configurable per hardware.
  const timeout = request.provider === "local" ? env.LOCAL_AI_TIMEOUT_MS : 180_000;
  ...
  const response = await client.chat.completions.create(
    {
      ...
      messages: [
        {
          role: "system",
          content: getExtractionSystemPrompt(request.knownDocumentTypes, {
