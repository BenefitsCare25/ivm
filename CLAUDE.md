...
- **Session items page**: fetches `detailData` + `comparisonResult` (including `fieldComparisons`) + all FWA `validationResults` for up to 50 items. `TrackedItemsTable` renders expandable rows with a **3-column layout** (`src/components/portals/expanded-row/`):
  ...
  - **Column 2 — Comparison & Alerts**: Full field comparison table (all rows, scrollable 450px), match/mismatch summary + match rate %, AI summary text, diagnosis pill, currency conversion notices, and FWA alerts. Grid layout `[0.8fr_2fr_1fr]` gives this column the most space.
  - **Column 3 — Document Viewer**: File selector chips (one at a time), inline blob-based viewer (images with click-to-zoom lightbox overlay + drag-to-pan, PDFs in iframe via `blob:` URL to bypass `X-Frame-Options: DENY`). 500px fixed height.
...
- **Comparison prompt rules** (`src/lib/ai/prompts-comparison.ts`): System prompt uses principle-based rules — no hardcoded examples. Shared constants `DIAGNOSIS_JSON_SCHEMA` and `DIAGNOSIS_RULES` are exported and imported by `prompt-builder.ts` — never duplicate diagnosis prompt text inline. Key rules: (1) ignore leading punctuation on IDs/invoice numbers (`#C313875` = `C313875`); (2) semantic parent-brand matching for organization names — if two provider names share root brand words and one is plausibly a branch/variant, treat as MATCH (e.g. "Raffles Medical Teleconsult" vs "Raffles Medical Singapore"); (3) for monetary amounts differing by a large factor (50x–100x), AI notes in `notes` that amounts may be in different currencies rather than a simple data error. Do NOT add hardcoded provider examples to the prompt — the rule is intentionally generic so the AI applies its own world knowledge.
- **Expanded row — AI Comparison & Alerts column**: Section header is "AI Comparison & Alerts". Shows match/mismatch summary, AI narrative summary, Diagnosis Assessment card, field comparison table, currency conversion notices, FWA alerts.
- **Diagnosis Assessment** (`DiagnosisAssessment` type in `src/types/portal.ts`): AI-produced structured diagnosis stored in `ComparisonResult.diagnosisAssessment` (JSONB). The AI comparison prompt instructs the model to assess diagnosis from ALL evidence — explicit document diagnosis first, portal diagnosis (employee-submitted, less reliable) second, inference from medications/procedures/specialist type third. Fields: `diagnosis`, `icdCode`, `source` ("document"|"portal"|"inferred"), `confidence`, `evidence`. Source badges: green "From Document", amber "From Portal", purple "AI Inferred". Stethoscope icon + ICD-10 code pill. Migration: `20260422100000_add_diagnosis_assessment`.
...
### Portal Tracker — Currency Detection Pipeline
- **Purpose**: Detect foreign currency amounts in PDF-extracted fields and look up SGD conversion rates (MAS or ExchangeRate-API fallback)
- **Backend**: `src/lib/validations/currency.ts` — `checkForeignCurrency(trackedItemId, pdfFields, portalData)`. Scans PDF fields via `parseCurrencyAmount()` from `src/lib/currency/detector.ts`. Creates `CURRENCY_CONVERSION` `ValidationResult` with metadata (originalCurrency, originalAmount, sgdAmount, rate, rateDate, source, isFallback, isFuture).
- **Critical: `pdfRawFields` vs `pdfFields`**: AI extraction produces two values per field — `field.value` (numeric only, e.g. `"56280.50"`) and `field.rawText` (original with currency symbols, e.g. `"PHP 56,280.50"`). The worker builds two maps: `pdfFields` (from `field.value`, for comparison) and `pdfRawFields` (from `field.rawText ?? field.value`, for currency detection). **Always pass `pdfRawFields` to `checkForeignCurrency()`** — `pdfFields` strips currency prefixes and `parseCurrencyAmount()` will never match.
- **Detector**: `src/lib/currency/detector.ts` — regex-based `parseCurrencyAmount()` requires explicit currency prefix/symbol (e.g. `PHP`, `₱`, `USD`, `MYR`, `RM`). Returns `null` for SGD or unrecognized patterns. `isAmountField()` and `isDateField()` helper regexes for field label matching.
- **UI**: `ComparisonColumn` renders `CURRENCY_CONVERSION` alerts in a dedicated "Currency Conversion" section with blue styling, showing original→SGD conversion, rate, date, and source badges (NEAREST DATE, ESTIMATED, LIVE).
